...
class Search
  ...
  def self.query(text,options = {})
    ...
    if type.blank?
      ...
      #      words.each do |word|
      #
      #        reg = "#{Search.key_prefix}#!#*#{word}*#!#Product"
      #        keys += $redis.keys(reg)[0,limit]
      #      end

      #  reg = "#{Search.key_prefix}#!#*#{words[1]}*#!#Product"
      #  keys += $redis.keys(reg)[0,limit]
    else
    ...
    puts reg
    puts "ddddddddddddfffffffffffffffffggggggggggggggggggggggg"
    puts keys
    puts "-------------------==================================="
    result = []
    return result if keys.blank?
    $redis.mget(*keys).each do |r|
      puts r
      puts "rrrrrrrrrrrrrrrrrrrrrrrrrrrr22222222222222222222"
      begin
        result << JSON.parse(r)
      rescue => e
        Rails.logger.info { "Search.query failed: #{e}" }
