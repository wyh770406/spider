...
#    URL = "http://list.tmall.com/search_product.htm?q=%C7%EF&type=p&cat=all&userBucket=15" 
#    def self.category_list
#      doc = Nokogiri::HTML(open(URL).read)
#      c_arr = []
#      doc.search("ul#J_CateItemList/li/a").each do |ele|
#        p ele["href"]
#        c_arr.concat(self.get_categories(ele["href"]))
#      end
#
#      c_arr
#    end
#
#    def self.get_categories(link)
#      arr = []
#      doc = Nokogiri::HTML(open(link).read)
#      doc.css("ul#J_CateItemList/li").each do |e|
#        if e.attributes["data-lazyload"].present?
#          self.get_categories e.search("a").first["href"]
#        else
#          e.search("a").each do |i|
#            name = i.text.gsub("\r\n", "").gsub("\t", "").gsub(/\(\d+\)/, "")
#            arr << {
#              :url => i["href"],
#              :name => name
...
#    end
		...
		URL = "http://www.tmall.com/go/act/tmall/iwanttobuy.php"
		
		def self.open_and_html(url)
			#puts url
			html = open(url).read.force_encoding("GB18030")
      html.encode!("UTF-8", :invalid => :replace, :undef => :replace, :replace => "?") 
			Nokogiri::HTML(html)
		end

		def self.get_big_cate
			doc = self.open_and_html(URL)
			doc.css(".oldCategoryBox h2 a").map{ |elem| elem[:href] }.compact.select{|str| str != '' && str.match(/^http/) }
		end

		def self.get_low_cate(url, arr=[])
			return arr unless url.to_s.match(/^http/)
			doc = self.open_and_html(url)
			doc.css(".cate-item-list li").each do |item|
				if !item['data-lazyload']
					arr << { :url => item.css('a').first[:href], :name => item.css('a').first.inner_text.to_s.strip.gsub(/\(.*\)/, '') } 
				else
					get_low_cate(item.css('a').first[:href], arr)
				end
			end
			return arr
		end
		...
		def self.category_list
			arr = []
			self.get_big_cate.each do |big_cate|
				arr += get_low_cate(big_cate)
			end
			return arr
		end
